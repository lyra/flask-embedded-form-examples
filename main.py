...
def process_ipn():
    ...
    return render_template(
        ...
        kr_public_key=behavior_parameters['test_public_key'],
...
def ipn():
    ...
    signature = service.compute_hmac_sha256_signature(behavior_parameters['test_password'], request.form.get('kr-answer'))
...
def redirect_():
    ...
    if request.args.get('status') == 'success':
        signature = service.compute_hmac_sha256_signature(
            behavior_parameters['test_hmac_sha_256_key'], request.form.get('kr-answer')
        )
        app.logger.info(json.dumps(json.loads(request.form.get('kr-answer').replace('"','\"')),indent=4))
...
def create_form_token(entry_body, url=None):
    ...
    test_password = behavior_parameters['test_password']
    string_to_encode = f"{shop_id}:{test_password}"
